...
class KCFTracker:
    def __init__(self, hog=False, fixed_window=True, multiscale=False):
        ...
        self.padding = 2.5 #2.0 #2.5  # extra area surrounding the target
        self.output_sigma_factor = 0.0625 #0.001#0.0625 #0.125  # bandwidth of gaussian target

        if (hog):  # HOG feature
            # VOT
            self.interp_factor = 0.012  #0.012  # linear interpolation factor for adaptation
            self.sigma = 0.2 #0.6  # gaussian kernel bandwidth
        ...
        if (multiscale):
            ...
            self.scale_step = 1.001#1.0005 #1.05 scale step for multi-scale estimation
            self.scale_weight = 0.90  # to downweight detection scores of other scales for added stability
        elif (fixed_window):
    ...
    def train(self, x, train_interp_factor):
        if self.peak > 0.8:
            ...
            self._tmpl_g = (1 - train_interp_factor) * self._tmpl_g + train_interp_factor * x
            self._alphaf_g = (1 - train_interp_factor) * self._alphaf_g + train_interp_factor * alphaf

            self._tmpl = self._tmpl_g
            self._alphaf = self._alphaf_g
        else:
            k = self.gaussianCorrelation(x, x)
            alphaf = complexDivision(self._prob, fftd(k) + self.lambdar)
            self._tmpl = (1 - train_interp_factor) * self._tmpl_g + train_interp_factor * x
            self._alphaf = (1 - train_interp_factor) * self._alphaf_g + train_interp_factor * alphaf
    ...
    def init(self, roi, image):
        ...
        self.peak = 1
        self._tmpl_g = self._tmpl.copy()
        self._alphaf_g = self._alphaf.copy()
    ...
    def update(self, image):
        ...
        cx = self._roi[0] + self._roi[2] / 2.
        cy = self._roi[1] + self._roi[3] / 2.
        #print("pixel displacement:{}___peak vlue{}".format((cx,cy),peak_value))
        
        return self._roi,cx,cy,peak_value